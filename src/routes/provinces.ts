...
const provinceRoutes: FastifyPluginAsync<ProvinceRouteOptions> = async (fastify, options) => {
  ...
  fastify.get<{ Querystring: ProvinceListQuery }>(
    ...
    async (request, reply) => {
      ...
      const ranges = validateRangeFilters(request.query, ['population', 'area', 'altitude']);
  ...
  fastify.get<{ Params: ProvinceParams; Querystring: PostalCodeStatusPaginationFieldQuery }>(
